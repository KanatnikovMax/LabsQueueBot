...
public class InlineKeyboardCreator
{
    ...
    /// <returns></returns>
    public static InlineKeyboardMarkup ListToKeyboard(
        List<string> list,
        bool isNeedAdd,
        bool isNeedBack,
        int collumnsCount)
    {
        ...
        if (isNeedAdd)
            arr[size] = [InlineKeyboardButton.WithCallbackData("Добавить")];
        if (isNeedBack)
            arr[size + (isNeedAdd ? 1 : 0)] = [InlineKeyboardButton.WithCallbackData("Назад")];
